...
env.local_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
print env.local_dir
...
#       'pygsm': 'git://github.com/mwana/pygsm',
        'django-tables': 'git://github.com/mwana/django-tables.git',
        'django-app-settings': 'git://github.com/mwana/django-app-settings.git',
    }
    setup_path()
...
def pull_and_checkout_all():
    for name, commit, repo, dest in iter_commits():
        if repo:
            run('cd %s && git pull && git checkout %s' %
                (dest, commit))
...
def bootstrap():
    ...
    install_init_script()
    clone_all()
    put('localsettings.py.example',
        '%s/mwana/localsettings.py' % env.path)
    pull_and_checkout_all()
